...
class Model_attributes extends CI_Model
{
	...
	public function getAttributeNameByValueId($value_id)
	{
	    $this->db->select('attributes.name');
	    $this->db->from('attribute_value');
	    $this->db->join('attributes', 'attributes.id = attribute_value.attribute_parent_id');
	    $this->db->where('attribute_value.id', $value_id);
	    $query = $this->db->get();
	    $row = $query->row_array();
	    return $row ? $row['name'] : null;
	}

	public function getAttributeDataByValueId($value_id)
	{
	    $this->db->select('attributes.name as attribute_name, attribute_value.value');
	    $this->db->from('attribute_value');
	    $this->db->join('attributes', 'attributes.id = attribute_value.attribute_parent_id');
	    $this->db->where('attribute_value.id', $value_id);
	    $query = $this->db->get();
